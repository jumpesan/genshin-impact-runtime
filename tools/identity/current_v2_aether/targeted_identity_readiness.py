...
SUPPORTED_CHARACTER_REGISTRY_SCHEMAS = frozenset({1, 2})


@dataclass(frozen=True)
class CharacterRegistrySnapshot:
    schema_version: int
    base_identity_kinds: dict[str, str | None]
...
def _load_character_registry(path: Path) -> CharacterRegistrySnapshot:
    ...
    if schema_version not in SUPPORTED_CHARACTER_REGISTRY_SCHEMAS:
        supported = ", ".join(
            str(item) for item in sorted(SUPPORTED_CHARACTER_REGISTRY_SCHEMAS)
        )
        raise ValueError(
            f"{path}: unsupported Character Identity Registry schema_version "
            f"{schema_version!r}; targeted interface supports {supported}"
        )
    ...
    for index, raw in enumerate(base_rows):
        ...
        if schema_version == 1:
            canonical_kind: str | None = shared._stringify(
                raw.get("identity_kind"),
                field_name="identity_kind",
                origin=origin,
            )
        else:
            base_status = shared._stringify(
                raw.get("base_identity_status"),
                field_name="base_identity_status",
                origin=origin,
            )
            if base_status != "accepted":
                raise ValueError(
                    f"{origin}: base_identity_status must be 'accepted', "
                    f"got {base_status!r}"
                )

            relationship = raw.get("relationship")
            if not isinstance(relationship, dict):
                raise ValueError(f"{origin}: relationship must be an object")
            relationship_status = shared._stringify(
                relationship.get("status"),
                field_name="relationship.status",
                origin=origin,
            )
            if relationship_status == "resolved":
                canonical_kind = shared._stringify(
                    relationship.get("kind"),
                    field_name="relationship.kind",
                    origin=origin,
                )
            elif relationship_status == "pending":
                canonical_kind = None
            else:
                raise ValueError(
                    f"{origin}: unsupported relationship.status "
                    f"{relationship_status!r}"
                )

        if canonical_id in base_identity_kinds:
            raise ValueError(f"{origin}: duplicate canonical_character_id {canonical_id}")
        base_identity_kinds[canonical_id] = canonical_kind
    ...
    *,
    ...
    if not report.error_count and registry is not None:
        ...
        if alias is None:
            ...
            if canonical_id not in registry.base_identity_kinds:
                report.error(
                    "character_registry_target_missing",
                    f"source {normalized_source_id} -> {canonical_id}; "
                    "target is not an accepted Production base identity in "
                    "Character Identity Registry",
                )
            else:
                canonical_kind = registry.base_identity_kinds[canonical_id]
                report.info(
